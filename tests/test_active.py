...
class TestUtils(TestCase):
    ...
    def test_compute_02(self):
        ...
        ss = ActiveSubspaces()
        ss.compute(gradients=gradients, weights=weights, method='exact', nboot=150)
        true_evals = np.array([[0.571596],
                               [0.465819],
                               [0.272198],
                               [0.175012]])
    ...
    def test_compute_03(self):
        ...
        weights = np.ones((15, 1)) / 15
        ss = ActiveSubspaces()
        ss.compute(gradients=gradients, weights=weights, method='exact', nboot=200)
    ...
    def test_compute_04(self):
        ...
        weights = np.ones((15, 1)) / 15
        ss = ActiveSubspaces()
        ss.compute(gradients=gradients, weights=weights, method='exact', nboot=250)
    ...
    def test_forward_01(self):
        ...
        true_active = np.array([[0.004748, 0.331107],
                                [0.949099, 0.347534]])
    ...
    def test_compute_bootstrap_ranges_02(self):
        ...
        weights = np.ones((30, 1)) / 30
        ss = ActiveSubspaces()
        ss.compute(gradients=gradients, weights=weights, method='exact', nboot=100)
