...
class ActiveSubspaces(Subspaces):
    ...
    def compute(self, inputs=None, outputs=None, gradients=None, weights=None, method='exact', nboot=100):
        """[summary]
        ...
        if method == 'exact':
            ...
            self.cov_matrix, self.evals, self.evects = self._build_decompose_cov_matrix(gradients=gradients, weights=weights, method=method)
            self._compute_bootstrap_ranges(gradients, weights, method=method, nboot=nboot)

        # estimate active subspace with local linear models.
        if method == 'local':
            if inputs is None or outputs is None:
                raise ValueError('inputs or outputs argument is None.')
            gradients = local_linear_gradients(inputs=inputs, outputs=outputs, weights=weights)
            if weights is None:
    ...
    @staticmethod
    def _build_decompose_cov_matrix(inputs=None, outputs=None, gradients=None, weights=None, method=None):
